...
## 3. [done] Provision kit identity over USB

Kiosks are created in the console (`New kiosk` + slot → `my-hro-kiosk-nn`) before a board is plugged in. Identify/Flash on that kiosk binds the ESP32 MAC. After a passing flash the bench sends `MHCFG hostname=my-hro-kiosk-nn` over serial (optional `BENCH_WIFI_SSID` / `BENCH_WIFI_PASSWORD`). Firmware must ACK `MHCFG OK` to store NVS; until then the captive portal still works.
...
## 5. [done] Pin the job to a chip, not a COM port
